...
function formatLuck(difficulty, shares, solo = false) {
  ...
  if (!percent) {
    return (
      `<span class="luckGood">?</span>` +
      (solo === true ? `<span class="fa fa-user luckGood" title="Solo Mined"></span>` : ``)
    );
  } else if (percent <= 100) {
    return (
      `<span class="luckGood">${percent}%&nbsp;</span>` +
      (solo === true ? `<span class="fa fa-user luckGood" title="Solo Mined"></span>` : ``)
    );
  } else if (percent >= 101 && percent <= 150) {
    return (
      `<span class="luckMid">${percent}%&nbsp;</span>` +
      (solo === true ? `<span class="fa fa-user luckMid" title="Solo Mined"></span>` : ``)
    );
  } else {
    return (
      `<span class="luckBad">${percent}%&nbsp;</span>` +
      (solo === true ? `<span class="fa fa-user luckBad" title="Solo Mined"></span>` : ``)
    );
  }
}
...
function getTransactionUrl(id, stats) {
  if (stats && blockExplorers) {
    return blockExplorers[stats.config.coin].transactionExplorer
...
function sortTable() {
  var table = $(this).parents('table').eq(0),
    rows = table
      .find('tr:gt(0)')
      .toArray()
      .sort(compareTableRows($(this).index()));
  this.asc = !this.asc;
  if (!this.asc) {
    rows = rows.reverse();
  }
...
function poolBlocks_GenerateChart(data, displayedChart) {
  if (
    displayedChart[data.config.coin] ||
    !data.charts.blocks ||
    data.charts.blocks === 'undefined' ||
    !data.charts.blocksSolo ||
    data.charts.blocksSolo === 'undefined'
  )
    return;
  ...
  if (chartDays) {
    if (chartDays === 1)
      title = getTranslation('blocksFoundLast24')
        ? getTranslation('blocksFoundLast24')
        : 'Blocks found in the last 24 hours';
    else
      title = getTranslation('blocksFoundLastDays')
        ? getTranslation('blocksFoundLastDays')
        : 'Blocks found in the last {DAYS} days';
...
function getBlockRowElement(block, jsonString, stats) {
  ...
  var columns =
    ...
    '</td>' +
    '<td class="col5">' +
    formatBlockLink(block.hash, stats) +
    '</td>' +
    '<td class="col5" title="Miners Address">' +
    block.address +
    '</td>' +
    '<td class="col6" align="right" title="' +
    block.shares +
    ' shares submitted">' +
    formatLuck(block.difficulty, block.shares, block.solo) +
    '</td>' +
    '<td class="col7">' +
    block.maturity +
    '</td>';
...
function poolBlocks_InitTemplate(ranOnce, displayedChart, xhrGetBlocks) {
  ...
  Object.keys(mergedStats).forEach((key) => {
    ...
    $(`#averageLuck${key}`).html(
      formatLuck(mergedStats[key].pool.totalDiff, mergedStats[key].pool.totalShares)
    );
...
function top10Miners_GetMinerCells(position, data) {
  ...
  return (
    '<td class="col1" data-sort="' +
    position +
    '">' +
    position +
    '</td>' +
    '<td class="col2" data-sort="' +
    miner +
    '">' +
    miner +
    '</td>' +
    '<td class="col3" data-sort="' +
    hashrate +
    '">' +
    getReadableHashRateString(hashrate) +
    '/sec</td>' +
    '<td class="col4" data-sort="' +
    lastShare +
    '">' +
    (lastShare ? $.timeago(new Date(parseInt(lastShare) * 1000).toISOString()) : 'Never') +
    '</td>' +
    '<td class="col5" data-sort="' +
    hashes +
    '">' +
    hashes +
    '</td>'
  ...
  xhrGetMiners[key] = $.ajax({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.length; ++i) {
        $(`#top10miners_rows${key}`).append(
          '<tr>' + top10Miners_GetMinerCells(i + 1, data[i]) + '</tr>'
        );
      }
    },
  });
...
function settings_SetEmailNotifications(stats, endPoint, email, address, ip, enable) {
  ...
  $.ajax({
    ...
    if (data.status == 'done') {
      if (enable) {
        settings_ShowSuccess(
          'notificationEnabled',
          'Done! Email notifications have been enabled',
          stats
        );
      } else {
        settings_ShowSuccess(
          'notificationDisabled',
          'Done! Email notifications have been disabled',
          stats
        );
      }
    } else {
...
function payments_InitTemplate(xhrGetPayments, ranOnce) {
  ...
  updateText(
    `paymentsMinimum${coin}`,
    getReadableCoin(lastStats, lastStats.config.minPaymentThreshold)
  );
  updateText(
    `paymentsDenomination${coin}`,
    getReadableCoin(lastStats, lastStats.config.denominationUnit, 3)
  );
  ...
  Object.keys(mergedStats).forEach((key) => {
    ...
    updateText(
      `paymentsMinimum${key}`,
      getReadableCoin(mergedStats[key], mergedStats[key].config.minPaymentThreshold)
    );
    updateText(
      `paymentsDenomination${key}`,
      getReadableCoin(mergedStats[key], mergedStats[key].config.denominationUnit, 3)
    );
...
function market_LoadMarketData(api, stats, loadedData, currencyPairs, xhrMarketGets, marketPrices) {
  ...
  if (typeof marketCurrencies !== 'undefined' && marketCurrencies.length > 0) {
    let intervalMarketPolling = setInterval(
...
function market_UpdateMarkets(api, stats, currencyPairs, xhrMarketGets, marketPrices) {
  ...
  for (let i = 0; i < marketCurrencies.length; i++) {
    currencyPairs[stats.config.coin].push(
      marketCurrencies[i].replace('{symbol}', stats.config.symbol).toUpperCase()
    );
  }
...
function market_RenderMarketPrice(base, target, price, source, stats, marketPrices) {
  ...
  if (sourceURL)
    source = '<a href="' + sourceURL + '" target="_blank" rel="nofollow">' + source + '</a>';
...
function market_CalcEstimateProfit(marketPrices) {
  ...
  try {
    ...
    if (profit) {
      ...
      updateText(
        `calcHashAmount${coin}2`,
        market_GetCurrencyPriceText(lastStats, profit, marketPrices)
      );
      //return;
    } else {
  ...
  Object.keys(mergedStats).forEach((key) => {
    try {
      ...
      let profit =
        ((hashRate * 86400) / mergedStats[key].network.difficulty) *
        mergedStats[key].lastblock.reward;
      if (profit) {
        updateText(`calcHashAmount${key}1`, getReadableCoin(mergedStats[key], profit));
        updateText(
          `calcHashAmount${key}2`,
          market_GetCurrencyPriceText(mergedStats[key], profit, marketPrices)
        );
        return;
      } else {
...
function market_GetCurrencyPriceText(stats, coinsRaw, marketPrices) {
  if (
    !priceCurrency ||
    !marketPrices[stats.config.coin] ||
    !marketPrices[stats.config.coin][priceCurrency]
  )
    return;
  let priceInCurrency =
    Math.trunc(
      getReadableCoin(stats, coinsRaw, 2, true) *
        marketPrices[stats.config.coin][priceCurrency] *
        100
    ) / 100;
...
function market_InitTemplate(
  ranOnce,
  chartsInitialized,
  loadedData,
  marketPrices,
  intervalChartsUpdate,
  currencyPairs,
  xhrMarketGets
) {
...
function workerstats_Setup(stats, api, addressTimeout, xhrAddressPoll, xhrGetPayments) {
  ...
  $(`#lookUp${stats.config.coin}`).click(function () {
    ...
    if (xhrAddressPoll[stats.config.coin]) xhrAddressPoll[stats.config.coin].abort();
  ...
  $(`#yourStatsInput${stats.config.coin}`).keyup(function (e) {
    if (e.keyCode === 13) $(`#lookUp${stats.config.coin}`).click();
  });
  ...
  xhrAddressPoll[stats.config.coin] = $.ajax({
    ...
    success: function (data) {
      ...
      if (data.stats.lastShare) {
        $(`#yourLastShare${stats.config.coin}`).timeago(
          'update',
          new Date(parseInt(data.stats.lastShare) * 1000).toISOString()
        );
      } // AQUÍ
      else {
      ...
      updateText(
        `yourHashrateHolder${stats.config.coin}`,
        (getReadableHashRateString(data.stats.hashrate) || '0 H') + '/sec'
      );
      if ('hashrate_1h' in data.stats) {
        $(`#minerAvgHR${stats.config.coin}`).show();
        updateText(
          `yourHR1h${stats.config.coin}`,
          (getReadableHashRateString(data.stats.hashrate_1h) || '0 H') + '/s'
        );
        updateText(
          `yourHR6h${stats.config.coin}`,
          (getReadableHashRateString(data.stats.hashrate_6h) || '0 H') + '/s'
        );
        updateText(
          `yourHR24h${stats.config.coin}`,
          (getReadableHashRateString(data.stats.hashrate_24h) || '0 H') + '/s'
        );
      } else {
      ...
      updateText(
        `yourPendingBalance${stats.config.coin}`,
        getReadableCoin(stats, data.stats.balance)
      );
      ...
      updateText(
        `yourRoundShareProportion${stats.config.coin}`,
        Math.round(share_pct * 1000) / 1000
      );
      updateText(
        `yourRoundScoreProportion${stats.config.coin}`,
        Math.round(score_pct * 1000) / 1000
      );
      if (!lastStats.config.slushMiningEnabled) {
...
function workerstats_GetWorkerRowElement(worker, jsonString, stats) {
  ...
  row.setAttribute(
    'id',
    'workerRow' + stats.config.coin + '_' + workerstats_GetWorkerRowId(worker.name)
  );
...
function workerstats_GetWorkerCells(worker) {
  ...
  return (
    ...
    status +
    '"><i class="fa fa-' +
    (status == 'ok' ? 'check status-ok' : 'times status-error') +
    '"></i></td>' +
    '<td class="col2" data-sort="' +
    (worker.name != 'undefined' ? worker.name : '') +
    '">' +
    (worker.name != 'undefined' ? worker.name : '<em>Undefined</em>') +
    '</td>' +
    '<td class="col3" data-sort="' +
    hashrate +
    '">' +
    getReadableHashRateString(hashrate) +
    '/s</td>' +
    '<td class="col4 avghr" data-sort="' +
    hashrate1h +
    '">' +
    getReadableHashRateString(hashrate1h) +
    '/s</td>' +
    '<td class="col5 avghr" data-sort="' +
    hashrate6h +
    '">' +
    getReadableHashRateString(hashrate6h) +
    '/s</td>' +
    '<td class="col6 avghr" data-sort="' +
    hashrate24h +
    '">' +
    getReadableHashRateString(hashrate24h) +
    '/s</td>' +
    '<td class="col4" data-sort="' +
    lastShare +
    '">' +
...
function workerstats_RenderWorkers(workersData, stats) {
  ...
  for (var i = 0; i < workersData.length; i++) {
    var existingRow = document.getElementById(
      `workerRow${stats.config.coin}_${workerstats_GetWorkerRowId(workersData[i].name)}`
    );
    if (!existingRow) {
      $workersRows.empty();
      break;
...
function workerstats_RenderPayments(paymentsResults, stats) {
  ...
  for (var i = 0; i < paymentsResults.length; i += 2) {
    ...
    var existingSummary = document.getElementById(
      `summaryRow${stats.config.coin}${summaryData.date}`
    );
  ...
  if (rawData) {
    for (var i = 0, xy; (xy = rawData[i]); i++) {
      ...
      graphData.values.push(
        fixValueToCoins ? getReadableCoin(lastStats, xy[1], null, true) : xy[1]
      );
    }
  }
...
function home_InitTemplate(parentStats, siblingStats) {
  $('#networkLastBlockFound').timeago(
    'update',
    new Date(parentStats.lastblock.timestamp * 1000).toISOString()
  );
  ...
  if ($(`#networkStats${coin}`).length == 0) {
    ...
    efforts.push({
      coin: coin,
      effort: `${((parentStats.pool.roundHashes / parentStats.network.difficulty) * 100).toFixed(
        1
      )}%`,
      symbol: parentStats.config.symbol,
    });
  ...
  updateText(
    `networkHashrate${coin}`,
    getReadableHashRateString(
      parentStats.network.difficulty / parentStats.config.coinDifficultyTarget
    ) + '/sec'
  );
  updateText(
    `networkDifficulty${coin}`,
    formatNumber(parentStats.network.difficulty.toString(), ' ')
  );
  updateText(`blockchainHeight${coin}`, formatNumber(parentStats.network.height.toString(), ' '));
  updateText(
    `networkLastReward${coin}`,
    getReadableCoin(parentStats, parentStats.lastblock.reward)
  );
  ...
  Object.keys(siblingStats).forEach((key) => {
    ...
    efforts.push({
      coin: key,
      effort: `${(
        (siblingStats[key].pool.roundHashes / siblingStats[key].network.difficulty) *
        100
      ).toFixed(1)}%`,
      symbol: siblingStats[key].config.symbol,
    });
    ...
    if (siblingStats[key].pool.lastBlockFound) {
      ...
      if (lastChildBlockFound > lastBlockFound) lastBlockFound = lastChildBlockFound;
    }

    updateText(
      `networkHashrate${key}`,
      getReadableHashRateString(
        siblingStats[key].network.difficulty / siblingStats[key].config.coinDifficultyTarget
      ) + '/sec'
    );
    updateText(
      `networkDifficulty${key}`,
      formatNumber(siblingStats[key].network.difficulty.toString(), ' ')
    );
    updateText(
      `blockchainHeight${key}`,
      formatNumber(siblingStats[key].network.height.toString(), ' ')
    );
    updateText(
      `networkLastReward${key}`,
      getReadableCoin(siblingStats[key], siblingStats[key].lastblock.reward)
    );
    updateText(
      `poolMiners${key}`,
      `${siblingStats[key].pool.miners}/${siblingStats[key].pool.minersSolo}`
    );
    updateText(
      `blocksTotal${key}`,
      `${siblingStats[key].pool.totalBlocks}/${siblingStats[key].pool.totalBlocksSolo}`
    );
    updateText(
      `currentEffort${key}`,
      ((siblingStats[key].pool.roundHashes / siblingStats[key].network.difficulty) * 100).toFixed(
        1
      ) + '%'
    );
  });
  ...
  updateText(
    'poolHashrateSolo',
    `SOLO: ${getReadableHashRateString(parentStats.pool.hashrateSolo)}/sec`
  );

  var hashPowerSolo =
    (parentStats.pool.hashrateSolo /
      (parentStats.network.difficulty / parentStats.config.coinDifficultyTarget)) *
    100;
  updateText('hashPowerSolo', hashPowerSolo.toFixed(2) + '%');

  var hashPower =
    (parentStats.pool.hashrate /
      (parentStats.network.difficulty / parentStats.config.coinDifficultyTarget)) *
    100;
  ...
  updateText(
    'paymentsMinimum',
    getReadableCoin(parentStats, parentStats.config.minPaymentThreshold)
  );

  updateText(
    'blockSolvedTime',
    getReadableTime(parentStats.network.difficulty / parentStats.pool.hashrate)
  );

  updateText(
    `currentEffort${coin}`,
    ((parentStats.pool.roundHashes / parentStats.network.difficulty) * 100).toFixed(1) + '%'
  );
}
